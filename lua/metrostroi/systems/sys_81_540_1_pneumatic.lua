...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_540_1_Pneumatic")
...
function TRAIN_SYSTEM:Initialize(parameters)
    ...
    self.DoorLineLeftPressure = 8.0 -- atm
    self.DoorLineRightPressure = 8.0 -- atm
    self.PrevDoorLineLeftPressure = 8.0 -- atm
    self.PrevDoorLineRightPressure = 8.0 -- atm
    if self.Train.SubwayTrain and self.Train.SubwayTrain.WagType == 1 then 
        self.Door2Pressure = 8.0 -- atm
        self.PrevDoor2Pressure = 8.0 -- atm
    end
    ...
    self.Train:LoadSystem("DoorCrane2","Relay","Switch")
    self.Train:LoadSystem("DoorCrane3","Relay","Switch")
    ...
    self.Train:LoadSystem("UAVAContact","Relay","Switch",{ bass=true})
    --Стояночный тормоз
    ...
    if not TURBOSTROI then
        ...
        self.DoorLeft = {false, false, false, false}
        self.DoorRight = {false, false, false, false}
        ...
        for i=1,#self.LeftDoorSpeed do
            if math.random() > 0.7 then
                ...
                --minimum = math.max(minimum, self.LeftDoorSpeed[i])
                --minimum = math.max(minimum, self.RightDoorSpeed[i])
            else
        ...
        for i=1,#self.LeftDoorSpeed do
            self.Train:SetPackedRatio("DoorL"..i.."Speed",minimum/self.LeftDoorSpeed[i])
            self.Train:SetPackedRatio("DoorR"..i.."Speed",minimum/self.RightDoorSpeed[i])
        end
    end
...
function TRAIN_SYSTEM:Outputs()
    return { "BrakeLinePressure", "BrakeCylinderPressure", "DriverValvePosition",
             "ReservoirPressure", "TrainLinePressure", "DoorLineLeftPressure", "DoorLineRightPressure", "WeightLoadRatio" }
end
...
function TRAIN_SYSTEM:Think(dT)
	...
	local DLPressure = Clamp(self.TrainLinePressure/2,0,1)*self.TrainLinePressure
	local DLLeftleak = self:equalizePressure(dT,"DoorLineLeftPressure", DLPressure, 3.0, 3.0, nil, 1.3)
    local DLRightleak = self:equalizePressure(dT,"DoorLineRightPressure", DLPressure, 3.0, 3.0, nil, 1.3)
    
    --if Train:GetWagonNumber() > 11000 then print(DLLeftleak, self.DoorLineLeftPressure) end
    if math.abs(self.DoorLineLeftPressure-self.PrevDoorLineLeftPressure) > 0.1 then --if math.abs(DLLeftleak) > 0.01 then
        self:equalizePressure(dT,"TrainLinePressure", self.DoorLineLeftPressure,0.6)
    end
    self.PrevDoorLineLeftPressure = self.DoorLineLeftPressure
    if math.abs(self.DoorLineRightPressure-self.PrevDoorLineRightPressure) > 0.1 then --if math.abs(DLRightleak) > 0.01 then
        self:equalizePressure(dT,"TrainLinePressure", self.DoorLineRightPressure,Train.DoorCrane1 and 0.5 or 0.6)
    end
    self.PrevDoorLineRightPressure = self.DoorLineRightPressure
    local leak = 0
    if Train.DoorCrane1 then
        local D2leak = self:equalizePressure(dT,"Door2Pressure",self.DoorLineRightPressure, 3.0, 3.0, nil, 1.3)
        if math.abs(self.Door2Pressure-self.PrevDoor2Pressure) > 0.1 then--if math.abs(D2leak) > 0.01 then
            self:equalizePressure(dT,"TrainLinePressure", self.Door2Pressure, 0.15)
        end
        self.PrevDoor2Pressure = self.Door2Pressure
        if Train.DoorCrane1.Value > 0.5 then
            leak = self:equalizePressure(dT,"Door2Pressure", 0.0,10.0,false,false,0.4)
        end
        Train:SetPackedRatio("Door2Pressure_dPdT", -leak/10)
    end
    leak = 0
    if Train.DoorCrane2.Value > 0.5 then
        leak = self:equalizePressure(dT,"DoorLineRightPressure", 0.0,10.0,false,false,0.4)
    end
    Train:SetPackedRatio("DoorLineRightPressure_dPdT", -leak/10)
    leak = 0
    if Train.DoorCrane3.Value > 0.5 then
        leak = self:equalizePressure(dT,"DoorLineLeftPressure", 0.0,10.0,false,false,0.4)
    end
    Train:SetPackedRatio("DoorLineLeftPressure_dPdT", -leak/10)
    ...
    if self.EmergencyValve then
        local leakst = 1.1*(Train:GetWagonCount())*Clamp(self.BrakeLinePressure/4,0,1)
    ...
    if Train.AirDistributorDisconnect.Value == 0 then
        ...
        if self.BrakeCylinderValve == 1 then
            self:equalizePressure(dT,"BrakeCylinderPressure", math.min(2.7 + self.WeightLoadRatio*1.3,targetPressure), 1+Clamp((self.BrakeCylinderPressure-0.5)/2.8,0,0.7), 3.50, nil, 0.8+Clamp((self.BrakeCylinderPressure-0.75)/0.6,0,1)) --0.75, 1.25)
        end
    ...
    if (self.BrakeCylinderPressure > 0.2 and self.BrakeCylinderPressure_dPdT > 0.1 or self.BrakeCylinderPressure_dPdT > 1) and not self.BrakeEngaged then
        ...
        Train:PlayOnce("brake","bass",1,Clamp(self.BrakeCylinderPressure_dPdT,0.7,1.2))
    end
    ...
    if Train.PneumaticNo2.Value == 0 then
        ...
        elseif self.BePN2 and self.BrakeCylinderPressure_dPdT > -0.2 then
            Train:PlayOnce("PN2end","bass",Clamp(math.min(1,(CurTime()-self.BePN2)/1.3)*((3.2-self.BrakeCylinderPressure)/1.2),0,1))
            self.BePN2 = false
        end
    end
    ...
    local PBPressure = Clamp(self.TrainLinePressure/5,0,1)*2.7
    ...
    if (Train.VDOL.Value == 1.0) and (Train.VDOP.Value == 0.0) and self.DoorLineLeftPressure > 3.5 then
        for i=1,4 do
            self.DoorLeft[i] = not Train["BlockDoors"..i.."2"]
        end
    end
    if (Train.VDOL.Value == 0.0) and (Train.VDOP.Value == 1.0) and self.DoorLineRightPressure > 3.5 then
        for i=1,4 do
            if Train.DoorCrane1 and i == 2 then
                if self.Door2Pressure > 3.5 then
                    self.DoorRight[2] = not Train["BlockDoors21"]
                end
            else
                self.DoorRight[i] = not Train["BlockDoors"..i.."1"]
            end
        end
    end
    if (Train.VDZ.Value == 1.0 or Train.VDOL.Value == 1.0 and Train.VDOP.Value == 1.0 or self.RZDTimer) then --and (DoorLeft or DoorRight) then
        if not self.OpenWaitL or CurTime()-self.OpenWaitL < 0.2 and self.DoorLineLeftPressure > 3.5 then
            for i=1,4 do
                self.DoorLeft[i] = false
            end
        end
        if not self.OpenWaitR or CurTime()-self.OpenWaitR < 0.2 and self.DoorLineRightPressure > 3.5 then
            for i=1,4 do
                if Train.DoorCrane1 and i == 2 then
                    if self.Door2Pressure > 3.5 then
                        self.DoorRight[2] = false
                    end
                else
                    self.DoorRight[i] = false
                end
            end
        end
    else
    ...
    local LeftDL = 1--Clamp(self.DoorLineLeftPressure-2.8, 0, 1)
    local RightDL = 1--Clamp(self.DoorLineRightPressure-2.8, 0, 1)
    if Train.CanStuckPassengerLeft then
        for i in ipairs(self.LeftDoorStuck) do
            self.LeftDoorStuck[i] = math.random() < (0.6+math.min(2,2-self.LeftDoorSpeed[i]/LeftDL)*0.2)*Train.CanStuckPassengerLeft*0.6 and (math.random() > 0.7 and CurTime()+math.random()*15)
        end
        Train.CanStuckPassengerLeft = false
    end
    if Train.CanStuckPassengerRight then
        for i in ipairs(self.RightDoorStuck) do
            self.RightDoorStuck[i] = math.random() < (0.6+math.min(2,2-self.RightDoorSpeed[i]/RightDL)*0.2)*Train.CanStuckPassengerRight*0.6 and (math.random() > 0.7 and CurTime()+math.random()*15)
        end
    ...
    local openR = true
    for i=1,4 do
        local leftdoorspeed = self.DoorLineLeftPressure > 3.5 and self.LeftDoorSpeed[i]/LeftDL or (Train["ChangeDoors"..i.."2"] and -1.5 or -1e9)
        local rightdoorspeed = self.DoorLineRightPressure > 3.5 and self.RightDoorSpeed[i]/RightDL or (Train["ChangeDoors"..i.."1"] and -1.5 or -1e9)
        local DoorsLeft = self.DoorLineLeftPressure > 3.5 and self.DoorLeft[i] or self.DoorLineLeftPressure <= 3.5 and Train["ChangeDoors"..i.."2Dir"]
        local DoorsRight = self.DoorLineRightPressure > 3.5 and self.DoorRight[i] or self.DoorLineRightPressure <= 3.5 and Train["ChangeDoors"..i.."1Dir"]
        if i == 2 and Train.DoorCrane1 then
            rightdoorspeed = self.Door2Pressure > 3.5 and rightdoorspeed or (Train["ChangeDoors21"] and -1.5 or -1e9)
            DoorsRight = self.Door2Pressure > 3.5 and self.DoorRight[i] or self.Door2Pressure <= 3.5 and Train["ChangeDoors21Dir"]
        end
        Train:SetPackedBool("DoorLChange"..i,leftdoorspeed>-10)
        Train:SetPackedBool("DoorRChange"..i,rightdoorspeed>-10) 
        self.LeftDoorDir[i] = Clamp(self.LeftDoorDir[i]+dT/(DoorsLeft and leftdoorspeed or -leftdoorspeed),-1,1)
        if leftdoorspeed == -1e9 or Train["BlockDoors"..i.."2"] then self.LeftDoorDir[i] = 0 end
        self.RightDoorDir[i] = Clamp(self.RightDoorDir[i]+dT/(DoorsRight and rightdoorspeed or -rightdoorspeed),-1,1)
        if rightdoorspeed == -1e9 or Train["BlockDoors"..i.."1"] then self.RightDoorDir[i] = 0 end
        self.LeftDoorState[i]  = Clamp(self.LeftDoorState[i] + ((self.LeftDoorDir[i]/leftdoorspeed)*dT),self.LeftDoorStuck[i] and 0.3 or 0,1)
        if self.LeftDoorState[i] == 0 or self.LeftDoorState[i] == 1 then self.LeftDoorDir[i] = 0 end
        self.RightDoorState[i]  = Clamp(self.RightDoorState[i] + ((self.RightDoorDir[i]/rightdoorspeed)*dT),self.RightDoorStuck[i] and 0.3 or 0,1)
        ...
        if self.LeftDoorState[i] > leftdoorspeed/20 then self.OpenWaitL = false end
        if self.RightDoorState[i] > rightdoorspeed/20 then self.OpenWaitR = false end
        ...
        Train:SetPackedRatio("DoorLPos"..i,self.LeftDoorState[i])
        Train:SetPackedRatio("DoorRPos"..i,self.RightDoorState[i])
        if self.LeftDoorStuck[i] and (DoorsLeft or type(self.LeftDoorStuck[i]) == "number" and CurTime()-self.LeftDoorStuck[i] > 0) then
            self.LeftDoorStuck[i] = false
        end
        if self.RightDoorStuck[i] and (DoorsRight or type(self.RightDoorStuck[i]) == "number" and CurTime()-self.RightDoorStuck[i] > 0) then
        ...
        Train:SetPackedRatio("DoorL"..i.."Speed",minimum/leftdoorspeed)
        Train:SetPackedRatio("DoorR"..i.."Speed",minimum/rightdoorspeed)
        Train:SetPackedBool("DoorL"..i,self.DoorLeft[i])
        Train:SetPackedBool("DoorR"..i,self.DoorRight[i])
    end
    ...
    Train.LeftDoorsOpening = self.DoorLeft[1] or self.DoorLeft[2] or self.DoorLeft[3] or self.DoorLeft[4]
    Train.RightDoorsOpening = self.DoorRight[1] or self.DoorRight[2] or self.DoorRight[3] or self.DoorRight[4]
