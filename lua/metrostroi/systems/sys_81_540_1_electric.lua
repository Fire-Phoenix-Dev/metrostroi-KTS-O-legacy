Metrostroi.DefineSystem("81_540_1_Electric")
...
function TRAIN_SYSTEM:SolveAllInternalCircuits(Train,dT,firstIter)
    ...
    Panel.P_11Power = B*Train.A44.Value
    S["B3"] = Panel.P_11Power*Train.P_11.Value
    ...
    Train.UAVAC:TriggerInput("Close",Train.UAVAContact.Value*(1-Train.UAVAC.Blocked))--??MS FIX
    ...
    S["Radio"] = ((Train:ReadTrainWire(10) + (BO*Train.A56.Value)) * Train.AR63.Value) + (B*Train.AV3.Value)
    Train.ECRS.Power = S["Radio"]
    Panel.RVS = S["Radio"]
    ...
    Panel.BUR = Train.ALSCoil.Enabled*Train.P_5.Value
    ...
    Panel.P_5Power = B*Train.A80.Value
    Train.BV:TriggerInput("Power",Panel.P_5Power*Train.P_5.Value)
    ...
    --FIXME ARS FMM2
    --F1->F2->BPF
    Panel.P_29Power = S["F7"] -- Питание БПФ300 (до предохранителя)
    S["F8"] = Panel.P_29Power*Train.P_29.Value*Train.L_4.Value
    Panel.Headlights1 = S["F8"]*Train.A46.Value
    Panel.Headlights2 = S["F8"]*Train.VUS.Value*Train.A47.Value
    S["F1"] = min(1,S["F1"]+T[71]*Train.OhrSig.Value*Train.Ring.Value)
    Panel.RedLight1 = S["F1"]*Train.A7.Value
    Panel.RedLight2 = S["F1"]*Train.A9.Value
    ...
    Panel.P_6Power = BO*Train.A65.Value    
    BPSN:TriggerInput("5x2",Panel.P_6Power*Train.P_6.Value*Train.KVP.Value)
...
function TRAIN_SYSTEM:SolveRKInternalCircuits(Train,dT,firstIter)
    ...
    local RK    = RheostatController.SelectedPosition
    local B     = (Train.Battery.Voltage > 55) and 1 or 0
    local BO    = B*Train.P_1.Value*Train.VB.Value
    local T     = Train.SolverTemporaryVariables
