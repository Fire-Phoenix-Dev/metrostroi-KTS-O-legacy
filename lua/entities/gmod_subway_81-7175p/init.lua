...
ENT.SyncTable = {
    "R_UNch","R_ZS","R_G","R_UPO","KVT","Ticker",
    "VZ1","VUD1","KDL","KDLK_mpl","KDLK_pbl","KDP","KDPK_mpl","KDPK_pbl","KRTD","KRTDK","DoorSelect","ASPS_RestartSetK",
    "KRZD","R_VPR","VozvratRP","AVU","KVP","ConverterProtection","RZP",--"SP","GreenRP",
    "VPAOn","VPAOff",
    "KSN","Ring","ARS","ALS","OtklAVU","TormAT","L_1","L_2","L_3","OhrSig","PVK",
    "VMKSwitch","BPSNSwitch","RezMK","ARS13","Radio13","L_4","VUS","VAH","VAD","KRP","OVT","HeadLightsSwitch",
    "EmergencyBrakeValve",
	...
	"ECRS_left","ECRS_right","ECRS_down", "ECRS_BRT","ECRS_up","ECRS_Cancel","ECRS_F1","ECRS_F2","ECRS_0","ECRS_1","ECRS_2","ECRS_3","ECRS_4","ECRS_5","ECRS_6","ECRS_7","ECRS_8","ECRS_9",
	"ASPS_View","ASPS_Esc","ASPS_Restart",
}
ENT.SyncFunctions = {
    ""
}
function ENT:Initialize()
    ...
    self:SetModel("models/metrostroi_train/81-5402/81-7175p_body.mdl")
    self.BaseClass.Initialize(self)
    self:SetPos(self:GetPos() + Vector(0,0,140))

    -- Create seat entities
    self.DriverSeat = self:CreateSeat("driver",Vector(417,0,-24.5))
	...
	self.PaxSeat1 = self:CreateSeat("instructor",Vector(-408.58,-45,-32),Angle(0,90,0)) ---408.58,40,-28+3
	self.PaxSeat2 = self:CreateSeat("instructor",Vector(-448.0,-45,-32),Angle(0,90,0)) ---448.0,40,-28+3
	...
	self.PaxSeat1:SetColor(Color(0,0,0,0))
    self.PaxSeat1:SetRenderMode(RENDERMODE_NONE)
	self.PaxSeat2:SetColor(Color(0,0,0,0))
    self.PaxSeat2:SetRenderMode(RENDERMODE_NONE)
	...
	self.chair_torec_1_bm = false
	self.chair_torec_2_bm = false
...
function ENT:TrainSpawnerUpdate()
    ...
    if typ == 1 then --PAKSDM
		...
		self:SetNW2Int("RingTypeOhr",math.ceil(math.random()*3))
    ...
    elseif typ == 2 then --PUAV
		...
		self:SetNW2Int("RingTypeOhr",puring)
    ...
    elseif typ == 3 then --PAM
		...
		self:SetNW2Bool("spb_reklama717")
    end

    local bpsn = 5
    self:SetNW2Int("BPSNType",bpsn)
    self:SetNW2Int("MaskType",not kvr and (8400<=num and num<=8599) and math.ceil(math.random()*3) or 3)
    self:SetNW2Bool("WhitePLights",true)
...
-- function ENT:OnUPOArrived()
    -- return self.KTS_O:TriggerInput("CheckUPO")
-- end
--------------------------------------------------------------------------------
function ENT:Think()
	...
	self:SetPackedBool("AO_ReadyLamp",self.ASPS.ASPSLamps.AO_Ready)
    self:SetPackedBool("AO_FireLamp",self.ASPS.ASPSLamps.AO_Fire)
    self:SetPackedBool("ReadyLamp",self.ASPS.ASPSLamps.Ready)
    self:SetPackedBool("ErrorLamp",self.ASPS.ASPSLamps.Error)
    self:SetPackedBool("PILamp",self.ASPS.ASPSLamps.PI)
    self:SetPackedBool("OSPLamp",self.ASPS.ASPSLamps.OSP)
    self:SetPackedBool("HV_OffLamp",self.ASPS.ASPSLamps.HVOff)

    self:SetPackedBool("PanelLights",self.Panel.PanelLights > 0.5)
	self:SetPackedBool("Svetolenta",self.Panel.PanelLights > 0.5)
    ...
    for i = 1,LampCount do
        if (lightsActive2 or lightsActive1) then
	...
	self:SetPackedBool("VZ1L",self.Panel.VZ1L > 0)
	self:SetPackedBool("VZ2L",self.Panel.VZ2L > 0)
	self:SetPackedBool("LMK",self.Panel.LMK > 0)
	self:SetPackedBool("LMKR",self.Panel.LMKR > 0)
	
	self:SetPackedBool("KNL",self.Panel.KNL > 0)
	...
	self:SetPackedBool("PTK",self.Panel.PTK > 0)

    self:SetPackedBool("LKVP",self.Panel.LKVP > 0)
	self:SetPackedBool("LKVPR",self.Panel.LKVPR > 0)
	self:SetPackedBool("KRUL",self.Panel.KRUL > 0)
    ...
    self:SetPackedBool("NMLow",self.Panel.NMLow > 0 and CurTime()%2.5>0.4)
    self:SetPackedBool("UAVATriggered",self.Panel.UAVATriggered > 0 and CurTime()%2.5>0.4)
	self:SetPackedBool("ABLow",self.Panel.ABLow > 0 and CurTime()%2.5>0.4)
	...
	self:SetPackedBool("chair_torec_1_cp",self.chair_torec_1_bm or self.PaxSeat1 and IsValid(self.PaxSeat1:GetDriver()))
	self:SetPackedBool("chair_torec_2_cp",self.chair_torec_2_bm or self.PaxSeat2 and IsValid(self.PaxSeat2:GetDriver()))
	...
	self:SetPackedRatio("BPSNSwitch",self.BPSNSwitch.Value/2)
	self:SetPackedRatio("HeadLightsSwitch",self.HeadLightsSwitch.Value/2)
	self:SetPackedRatio("VMKSwitch",self.VMKSwitch.Value/2)
...
function ENT:OnButtonPress(button,ply)
	...
	if button == "chair_torec_1_bm" then self.chair_torec_1_bm = not self.chair_torec_1_bm end
	if button == "chair_torec_2_bm" then self.chair_torec_2_bm = not self.chair_torec_2_bm end
	...
	if button == "KRTD" and not self.Plombs.KRTD then
        self.KRTDK:TriggerInput("Open",1)
        self.KRTD:TriggerInput("Close",1)
    end
    ...
    if (button == "VDL") or (button == "KDL") then
        ...
        self.KDLK_mpl:TriggerInput("Open",1)
		self.KDLK_pbl:TriggerInput("Open",1)
    end
    if (button == "KDP") then
        self.DoorSelect:TriggerInput("Close",1)
        self.KDPK_mpl:TriggerInput("Open",1)
		self.KDPK_pbl:TriggerInput("Open",1)
    end
...
function ENT:OnButtonRelease(button)
	...
	if button == "VMKSwitch-" then
        if self.VMKSwitch.Value == 0 then
            self.VMKSwitch:TriggerInput("Set",1)
        end
    end
end
