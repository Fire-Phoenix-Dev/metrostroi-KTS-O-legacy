...
ENT.SyncTable = {
    ...
    "EmergencyBrakeValve","DoorCrane1","DoorCrane2","DoorCrane3",
    ...
    "A58","A59","A61","A15","A66","A78",
    "P_1","P_5","P_6","P_11","P_29","P_1K","P_5K","P_6K","P_11K","P_29K",
    "KTSO_P",
    "RC1","VB","VRD","PB", "UAVA",
...
local function visibleoff(vehicle)
...
function ENT:Initialize()
    ...
    self:SetModel("models/metrostroi_train/81-5401/5401_ext1.mdl")--"models/metrostroi_train/81-717/81-717_spb.mdl")
    self.BaseClass.Initialize(self)
    self:SetPos(self:GetPos() + Vector(0,0,140))

    -- Create seat entities
    self.DriverSeat = self:CreateSeat("driver",Vector(431,7,-25))
    self.InstructorsSeat = self:CreateSeat("instructor",Vector(428,45,-25),Angle(0,270-8.3,0))
    self.ExtraSeat1 = self:CreateSeat("instructor",Vector(415.7,25,-43),Angle(0,90,0),"models/vehicles/prisoner_pod_inner.mdl")
    self.ExtraSeat2 = self:CreateSeat("instructor",Vector(430,-25,-43),Angle(0,90,0),"models/vehicles/prisoner_pod_inner.mdl")
    self.ExtraSeat3 = self:CreateSeat("instructor",Vector(395,45,-25),Angle(0,0,0))--,"models/vehicles/prisoner_pod_inner.mdl")

    -- Hide seats
    visibleoff(self.DriverSeat)
    visibleoff(self.InstructorsSeat)
    visibleoff(self.ExtraSeat1)
    visibleoff(self.ExtraSeat2)
    visibleoff(self.ExtraSeat3)
    ...
    if Metrostroi.BogeyOldMap then
        ...
        self.FrontCouple = self:CreateCouple(Vector( 432,0,-62),Angle(0,0,0),true,"722")
        ...
        self.FrontCouple = self:CreateCouple(Vector( 432,0,-66),Angle(0,0,0),true,"722")
    ...
    self.Lights = {
        ...
        -- Reverse
        [8] = { "light",Vector(482,-45, 52), Angle(0,0,0), Color(255,50,50),     brightness = 0.2, scale = 2.5, texture = "sprites/light_glow02.vmt"  },
        [9] = { "light",Vector(482, 45, 52), Angle(0,0,0), Color(255,50,50),     brightness = 0.2, scale = 2.5, texture = "sprites/light_glow02.vmt"  },

        -- Cabin
        [10] = { "dynamiclight",        Vector( 435, 0, 30), Angle(0,0,0), Color(255,245,245), distance = 550, brightness = 0.25},--216,161,92
        ...
        [31]  = { "light",           Vector(482,-45, -28), Angle(0,0,0), Color(255,220,180), brightness = 0.12, scale = 1.5, texture = "sprites/light_glow02.vmt" },
        [30]  = { "light",           Vector(482,-20, -28), Angle(0,0,0), Color(255,220,180), brightness = 0.12, scale = 1.5, texture = "sprites/light_glow02.vmt" },
        [32]  = { "light",           Vector(482, -7, -28), Angle(0,0,0), Color(255,220,180), brightness = 0.12, scale = 1.5, texture = "sprites/light_glow02.vmt" },
        [33]  = { "light",           Vector(482,  7, -28), Angle(0,0,0), Color(255,220,180), brightness = 0.12, scale = 1.5, texture = "sprites/light_glow02.vmt" },
        [34]  = { "light",           Vector(482, 20, -28), Angle(0,0,0), Color(255,220,180), brightness = 0.12, scale = 1.5, texture = "sprites/light_glow02.vmt" },
        [35]  = { "light",           Vector(482, 45, -28), Angle(0,0,0), Color(255,220,180), brightness = 0.12, scale = 1.5, texture = "sprites/light_glow02.vmt" },
    ...
    self.cab_cover1 = false
    self.int_cover3 = false
    self.int_cover4 = false
    
    for k=1,2 do
        for i=1,4 do
            self["ChangeDoors"..i..k] = false
            self["ChangeDoors"..i..k.."Dir"] = false
            self["BlockDoors"..i..k] = false
        end
    end

    self.PassengerIntSeat = false
    self.PassengerCabSeat = false
    ...
    self:SetNW2String("Texture","5401")
    self:SetNW2Int("Dest", math.random(0,12))
...
function ENT:TrainSpawnerUpdate()
    ...
    if not IsValid(self.LightSensor) then
        self.LightSensor = self:AddLightSensor(Vector(0,0,0),Angle(0,0,0),"models/metrostroi_train/81-5401/5401_cab_rfid_reader.mdl")
        --self.LightSensor:SetColor(Color(0,0,0,0))
        --self.LightSensor:SetRenderMode(RENDERMODE_TRANSALPHA)        
    end
    ...
    self:SetNW2String("CabTexture","Def_Yellow")
...
function ENT:Think()
    ...
    self:SetPackedBool("RedLight1",self.Panel.RedLight1 > 0)
    self:SetPackedBool("RedLight2",self.Panel.RedLight2 > 0)
    self:SetPackedBool("CabLights",self.Panel.CabLights>0)
    self:SetPackedBool("EqLights",self.Panel.EqLights>0)
    
    -- Interior/cabin lights
    self.CabLight = math.min(1.0, math.max(0.0, self.CabLight + self.CabLightV * (self.DeltaTime or 0)))
	--if self.CabLightV != 0.0 then RunConsoleCommand("say", self.CabLight) end
    self:SetPackedRatio("CabLight",self.CabLight)
    if (self.Panel.EqLights > 0.5) and (self.Panel.CabLights > 0.5) then
        self:SetLightPower(10,true,1.0*self.CabLight)
    elseif (self.Panel.CabLights > 0.5) then
        self:SetLightPower(10,true,0.1)--*self.CabLight)
    elseif self.Panel.EqLights > 0.5 then
        self:SetLightPower(10,true,0.6*self.CabLight)
    else
    ...
    for i=0,2 do
        self:SetLightPower(30+2*i,self.Panel.Headlights2>0,brightness)
        self:SetLightPower(31+2*i,self.Panel.Headlights1>0,brightness)
    end
    self:SetLightPower(8,self.Panel.RedLight1>0,1)
    self:SetLightPower(9,self.Panel.RedLight2>0,1)
    --local lightsActive1 = self.Panel.EmergencyLights > 0
    --local lightsActive2 = self.Panel.MainLights > 0.0
    local lightsActive = self.Panel.EmergencyLights*0.4+self.Panel.MainLights*0.6
    self:SetPackedRatio("lightsActive", lightsActive)
    local mul = 0
    local LampCount  = 14
    for i = 1,LampCount do
        if lightsActive > 0 then
	...
	--SOPD
	self:SetPackedBool("SOPD",self:ReadTrainWire(68) > 0)
    -- SD
    ...
    self:SetPackedBool("PAPower",self.PAM.State~=0)
    -- KT
    ...
    self:SetPackedBool("RVSLight",self.Panel.RVSLight > 0)
    self:SetPackedBool("BUR",self.Panel.BUR > 0)
    ...
    self:SetPackedBool("KTS_Power", self.KTS_O.State > 0)
    ...
    self:SetPackedBool("cab_cover1",self.cab_cover1)
    self:SetPackedBool("int_cover3",self.int_cover3)
    self:SetPackedBool("int_cover4",self.int_cover4)
    self:SetPackedBool("PassengerIntSeat",self.PassengerIntSeat or self.ExtraSeat3 and IsValid(self.ExtraSeat3) and IsValid(self.ExtraSeat3:GetDriver()))
    self:SetPackedBool("PassengerCabSeat",self.PassengerCabSeat or self.InstructorsSeat and IsValid(self.InstructorsSeat) and IsValid(self.InstructorsSeat:GetDriver()))
    ...
    self:SetPackedBool("Vent1Work",self.PowerSupply.X2_2 > 0)
...
function ENT:OnButtonPress(button,ply)
    ...
    if string.find(button,"P_") and not string.find(button,"K") and IsValid(ply) then 
        if self.Panel[button:gsub("Toggle","").."Power"] > 0 then
            ply:Kill() 
            sound.Play("ambient/energy/zap"..math.random(1,3)..".wav",ply:GetPos(),75,math.random(100,150),1.0)
        end
    end
    if button:find("Doors") then
        local DoorState = self.Pneumatic[(button[-1] == "1" and "Right" or "Left").."DoorState"][tonumber(button[-2])]
        if button:find("Change") then
            if DoorState == 0 then self[button.."Dir"] = true end
            if DoorState == 1 then self[button.."Dir"] = false end
            self[button] = true
        end
        if button:find("Block") then
            if DoorState == 0 then
                self[button] = not self[button]
                self:SetPackedBool(button,self[button])
            end
        end
    end
    ...
    if button == "PassengerDoor" then 
        if not self.PassengerIntSeat or self.ExtraSeat3 and IsValid(self.ExtraSeat3) and IsValid(self.ExtraSeat3:GetDriver()) then
            self.PassengerDoor = not self.PassengerDoor
            if self.PassengerDoor and self.ExtraSeat3 and IsValid(self.ExtraSeat3) then
                self.ExtraSeat3:SetSolid(SOLID_NONE)
            elseif self.ExtraSeat3 and IsValid(self.ExtraSeat3) then
                self.ExtraSeat3:SetSolid(SOLID_VPHYSICS)
            end
        end
    end
    if button == "CabinDoor" then
        if not self.PassengerCabSeat or self.InstructorsSeat and IsValid(self.InstructorsSeat) and IsValid(self.InstructorsSeat:GetDriver()) then
            self.CabinDoor = not self.CabinDoor
            if self.CabinDoor and self.InstructorsSeat and IsValid(self.InstructorsSeat) then
                self.InstructorsSeat:SetSolid(SOLID_NONE)
            elseif self.InstructorsSeat and IsValid(self.InstructorsSeat) then
                self.InstructorsSeat:SetSolid(SOLID_VPHYSICS)
            end
        end
    end
    ...
    if button == "cab_cover1" and self.P_11K.Value+self.P_1K.Value+self.P_5K.Value+self.P_6K.Value == 4 then self.cab_cover1 = not self.cab_cover1 end
    if button == "int_cover3" then self.int_cover3 = not self.int_cover3 end
    if button == "int_cover4" then self.int_cover4 = not self.int_cover4 end
    if button == "PassengerIntSeat" then
        if self.PassengerDoor then
            
        elseif not (self.ExtraSeat3 and IsValid(self.ExtraSeat3) and IsValid(self.ExtraSeat3:GetDriver())) then
            self.PassengerIntSeat = not self.PassengerIntSeat 
        end
    end
    if button == "PassengerCabSeat" then
        if self.CabinDoor then
        
        elseif not (self.InstructorsSeat and IsValid(self.InstructorsSeat) and IsValid(self.InstructorsSeat:GetDriver())) then
            self.PassengerCabSeat = not self.PassengerCabSeat
        end      
    end
	if button == "CabLight+" then
		self.CabLightV = 0.333
	end
	if button == "CabLight-" then
		self.CabLightV = -0.333
	end
...
function ENT:OnButtonRelease(button)
    if button:find("ChangeDoors") then
        self[button] = false
    end
    if string.find(button,"PneumaticBrakeSet") then
	...
	if button == "CabLight+" or button == "CabLight-" then
		self.CabLightV = 0.0
	end
